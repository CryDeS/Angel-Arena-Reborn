...
//	$.Msg( playerPanelName );
	...
	playes_panels[playerId] = playerPanel
...
//	$.Msg( playersContainer );
	...
	var kl = CustomNetTables.GetTableValue( "game_info", "kill_limit" )
	
	var total_str = teamDetails.team_score;
	if(kl && kl["kl"])
	{
		total_str =total_str + "/" + kl["kl"]
	}
	
	_ScoreboardUpdater_SetTextSafe( teamPanel, "TeamScore", total_str )
...
function TeamChange(luatable)
{
	var playerId = luatable.playerid
	var playerPanelName = "_dynamic_player_" + playerId;
	if ($("#"+ playerPanelName))
	{
		$("#" + playerPanelName).DeleteAsync(0.0);
	}
	if (playes_panels[playerId])
	{
		playes_panels[playerId].DeleteAsync(0.0)
	}
	/*var scoreboardConfig
	var scoreboardPanel
	ScoreboardUpdater_InitializeScoreboard(scoreboardConfig, scoreboardPanel)*/
	//$.Msg("Update team panel (try)")
}
...
(function()
{
	GameEvents.Subscribe( "custom_player_change_team", TeamChange)
	//$.Msg("TEST XXXX DANGER ALARM")
})();